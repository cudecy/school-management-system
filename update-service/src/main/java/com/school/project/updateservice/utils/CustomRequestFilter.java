package com.school.project.updateservice.utils;
                    ...
                    if (responseBody != null) {
                        if (responseBody.getResponseCode().equals(99)) {
                            ...
                            if(queryString.contains("=")) {
                                Long studentId = Long.parseLong(queryString.split("=")[1]);
                                ObjectMapper om = new ObjectMapper();
                                String responseDataString = om.writeValueAsString(responseBody.getResponseData());
                                AppUser currentUser = om.readValue(responseDataString.getBytes(), AppUser.class);
                                if(currentUser.getId().equals(studentId))
                                    filterChain.doFilter(servletRequest, servletResponse);
                                else
                                    populateServletResponse("Oops! You cannot edit another student's profile", servletResponse);
                            }
                        }else {
